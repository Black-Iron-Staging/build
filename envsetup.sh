...
function blackironupload() {
    read -p "Enter your SourceForge username: " sf_username
    target_device="$(get_build_var TARGET_DEVICE)"
    package_type="$(get_build_var BLACKIRON_PACKAGE_TYPE)"
    blackironversion="$(get_build_var BLACKIRON_VERSION)"
    blackironversion="${blackironversion%.*}.x"
    product_out="out/target/product/$target_device/"
    source_file="$(find "$product_out" -maxdepth 1 -type f -name 'Blackiron-*.zip' -print -quit)"
    
    if [ -z "$source_file" ]; then
        echo "Error: Could not find Blackiron zip file in $product_out"
        return 1
    fi
    
    filename="$(basename "$source_file" .zip)"
    destination="${sf_username}@frs.sourceforge.net:/home/frs/project/black-iron-project/$blackironversion/$package_type/$target_device/"
...
function blkilunch() {
    ...
    if [ -z "$device" ]; then
        if [[ -n "$TARGET_PRODUCT" ]]; then
            ...
            echo "Correct usage: blkilunch <device_codename> [build_type]"
...
function ascend() {
    if [[ -z "$TARGET_PRODUCT" ]]; then
        echo "Error: No device target set. Please use 'blkilunch' or 'lunch' to set the target device."
        return 1
    fi

    echo "ascend is deprecated. Please use blki instead."
    echo "Usage: blki [b|fb]"
    echo "   b   - Build bacon"
...
function blki() {
    if [[ "$1" == "help" ]]; then
        echo "Usage: blki [b|fb|sb] [-j<num_cores>]"
        echo "   b   - Build bacon"
        echo "   fb  - Fastboot update"
    ...
    case "$cmd" in
        sb)
            ...
            echo "Reminder: Please ensure that you have generated keys using 'gk -f' before running 'blki sb'."
...
function add_remote() {
    local remote_name="$1"
    local remote_url="$2"
    local manifest_path="android/snippets/blki.xml"
    local exclusion_list=("android" "OTA" "packages/apps/FaceUnlock" "vendor/gms")
...
function force_push() {
    ...
    local manifest_path="android/snippets/blki.xml"
    local exclusion_list=("android" "OTA" "packages/apps/FaceUnlock" "vendor/gms" "frameworks/base")
...
function generate_keys() {
    local subject="/C=US/ST=California/L=Los Angeles/O=Blackiron/OU=Blackiron/CN=Blackiron"
...
function sign_build() {
    local blackironbuild_version="$(get_build_var BLACKIRON_BUILD_VERSION)"
    local blackironversion="$(get_build_var BLACKIRON_VERSION)"
    local blackironcodename="$(get_build_var BLACKIRON_CODENAME)"
    local blackironpackage_type="$(get_build_var BLACKIRON_PACKAGE_TYPE)"
    local blackironrelease_type="$(get_build_var BLACKIRON_RELEASE_TYPE)"
    ...
    local target_file="$OUT/Blackiron-$blackironbuild_version-ota-signed.zip"
    ...
    echo "Creating Blackiron JSON OTA..."
    $ANDROID_BUILD_TOP/vendor/blackiron/build/tools/createjson.sh "$target_device" "$OUT" "Blackiron-$blackironbuild_version-ota-signed.zip" "$blackironversion" "$blackironcodename" "$blackironpackage_type" "$blackironrelease_type"
    local json_file="${blackironpackage_type}_${target_device}.json"
    cp -f "$OUT/$json_file" "OTA/$json_file"
    echo "Blackiron JSON OTA created and copied."
}
...
function extractSI() {
    ...
    unzip $OUT/Blackiron-$blackironbuild_version-ota-signed.zip -d $OUT/signed_builds_images
...
function rcleanup() {
    ...
    for manifest in .repo/manifests/default.xml .repo/manifests/snippets/crdroid.xml .repo/manifests/snippets/lineage.xml .repo/manifests/snippets/pixel.xml .repo/manifests/snippets/blackiron.xml; 
    do
        if [ -f "$manifest" ]; then
